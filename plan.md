...
2. **Hosting:** set `beatblox.downloadUrl` / `beatblox.hubKey`, generate the release key, publish APK + source (AGPL requires the source to be offered).
3. ✅ **Feedback server:** integrated with the Coreworkbench hub (`POST /api/feedback`, per-app key via `beatblox.hubKey`). Register the app at `/admin/apps`, add `projects/beatblox.yaml` in the hub repo, send one test report from a dev build.
